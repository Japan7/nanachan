...
class CustomHelpCommand(HelpCommand):
    ...
    async def send_bot_help(self, mapping: Mapping[Optional[Cog], List[Command]]):
        first_embed = Embed(title=f'{self.bot_name} commands manual', description='')
        embeds = []
        ...
        for cog in sorted(cogs, key=attrgetter('qualified_name')):
            ...
            if cog_embeds:
                ...
                first_embed.add_field(name=title, value=desc, inline=False)

                embeds.extend(cog_embeds)

        embeds.insert(0, first_embed)
        pages = [{'embed': embed} for embed in embeds]
        await HelpReactionListener.create(
            self.context.bot, self.get_destination().send, pages=pages
        )
    ...
    async def send_cog_help(self, cog: Cog):
        ...
        if pages:
            await NavigatorView.create(
                cast('Bot', self.context.bot), self.get_destination().send, pages=pages
            )
        else:
    ...
    async def send_group_help(self, group: Group):
        ...
        if help:
            embeds = [
                Embed(title=group.qualified_name, description=description)
    ...
    async def send_command_help(self, command: Command):
        ...
        if help:
            embeds = [
                Embed(description=description) for description in self.split_descriptions(help)
            ]
            pages = [{'embed': embed} for embed in embeds]
            await NavigatorView.create(
                cast('Bot', self.context.bot), self.get_destination().send, pages=pages
