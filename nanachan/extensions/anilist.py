...
class Anilist(Cog):
    ...
    async def _register_username(self, service: AnilistService, interaction: discord.Interaction):
    ...
    async def register(self, interaction: discord.Interaction):
        ...
        al_button = Button(
            emoji=self.bot.get_emoji_str('amoesip'), label='AniList', style=ButtonStyle.blurple
        )
        al_button.callback = partial(self._register_username, AnilistService.ANILIST)
        ...
        mal_button.callback = partial(self._register_username, AnilistService.MYANIMELIST)
        view.add_item(mal_button)

        await interaction.response.edit_message(view=None)

        await interaction.followup.send(
            content=f'{interaction.user.mention}\nSelect your tracking service', view=view
        )
    ...
    async def get_accounts(self, ctx: commands.Context[Bot]):
        ...
        for anilist in anilists:
            ...
            llist.append(EmbedField(str(member), f'[{anilist.service.value}]({link})'))
    ...
    @app_commands.autocomplete(search=media_autocomplete(media_type=MediaType.ANIME))
    @legacy_command()
    ...
    @app_commands.autocomplete(search=media_autocomplete(media_type=MediaType.MANGA))
    @legacy_command()
    ...
    async def ssearch(self, ctx: LegacyCommandContext, search: str):
        ...
        await NavigatorView.create(self.bot, ctx.reply, pages=[staff_page(s) for s in results])
