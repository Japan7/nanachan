...
from nanachan.utils.ollama import OllamaClient
...
class AI(NanaGroupCog, group_name='ai'):
    ...
    async def generate_discussion_title(self, content: str):
        ...
        gen = await self.ollama.generate_typed(prompt, stream=False, system='')
    ...
    async def chat_stream_blocks(self, messages: list[Message]):
        buf = ''
        async for part in await self.ollama.chat_typed(messages, stream=True):
